...
class Tab(QWidget):
    ...
    def __init__(self, parent):
        ...
        self.update_thread.progress.connect(lambda text: self.read_only_text.append(text))
    ...
    def diff(self):
        ...
        self.kahjumid.container_layout.addWidget(
            QLabel("<b>Arvestades kogu kasumist maha kogu kahjumi, jääb {} {} eurot".format(
                self.profit_or_not, abs(sum(self.kogu_kasum) - sum(self.kogu_kahjum)))))
        self.kasumid.container_layout.addWidget(
            QLabel("<b>Arvestades kogu kasumist maha kogu kahjumi, jääb {} {} eurot".format(
                self.profit_or_not, abs(sum(self.kogu_kasum) - sum(self.kogu_kahjum)))))
...
class UpdateThread(QThread):
    ...
    def collect_data(self):
        ...
        try:
            ...
            for sneaker_div in nike_object.find_all("div", class_="grid-item fullSize"):
                ...
                self.progress.emit("Collecting Nike product data {}".format(image_path))
            ...
            for gucci_div in gucci_object.find_all("div", class_="product-tiles-grid-item-image"):
                for image_tag in gucci_div.find_all("img"):
                    ...
                    self.progress.emit("Collecting Gucci product data {}".format(image_path))
                    if len(data) > 45:
                        data = data[:45]
    ...
    def add_data(self):
        ...
        self.progress.emit("Finished adding content into the database!")

        for filename in os.listdir("images/"):
            os.rename("images/" + filename, "images/" + filename + ".jpeg")

    def refresh_database(self):
        self.progress.emit("Refreshing database")
        try:
            with open("log.txt", "a") as debug:
                connection = sqlite3.connect("inventuur.db")
